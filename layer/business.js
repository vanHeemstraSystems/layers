 ...
 *
 * node business.js {\"uuid\":\"6e8bc430-9c3a-11d9-9669-0800200c9a66\"}
 *
 ...
 */
console.log('layer business - called');
...
console.log('layer business - _proxies: ', _proxies);                                                                                              // function () { return new Proxies(); }
...
join(_proxies(), function(proxies) {
  console.log('layer business - proxies: ', proxies); // Works: Proxies {}
  return(proxies);
}) //eof join proxies
  ...
  process.argv.forEach(function (val, index, array) {
    ...
    switch(index) {
      ...
      case 2: // optional additional command line argument
        console.log('layer business - additional command: ', val);
        try {
	      ...
	      if (o && typeof o === "object" && o !== null) {
            ...
            console.log('layer business - object: ', o);
            uuid = o.uuid;
            console.log('layer business - uuid: ', uuid);
...
//            console.log('server - _proxies().proxy().resources().resource()._6e8bc430_9c3a_11d9_9669_0800200c9a66(): ', _proxies().proxy().resources().resource()._6e8bc430_9c3a_11d9_9669_0800200c9a66());
            ...
            console.log('layer business - resource: ', resource);
            for (var key in resource) {
            	console.log('layer business - key: ', key);
            	// strip prefix _ if present on key, then substitute all _ for - if present on key
                var keyUuid = key.replace(/^\_/, "").replace(/_/g, "\-");
                console.log('layer business - keyUuid: ', keyUuid);
                if(uuid == keyUuid) {
                  console.log('layer business - uuid == keyUuid');
                  // do something
                  resourceForUuid = resource[key]();
                  break;
                }
			}
            console.log('layer business - resourceForUuid: ', resourceForUuid);
          } // eof if
        } // eof try
        catch (e) { 
          console.log('layer business - error: ', e);
        } // eof catch
        break; // eof case 2
      default:
        // do nothing
...
.then(function(resourceForUuid) {
  console.log('layer business - resourceForUuid: ', resourceForUuid); // Works: e.g. _6e8bc430_9c3a_11d9_9669_0800200c9a66 { URI: 'urn:uuid:6e8bc430-9c3a-11d9-9669-0800200c9a66' }

  var main = _proxies().proxy().mains().main();
  console.log('layer business - main: ', main);
  main.setproxies(_proxies);

  console.log('layer business - main.proxies(): ', main.proxies());

  main.setresource(resourceForUuid); 

  console.log('layer business - main.resource(): ', main.resource());
  
  // Start of the run chain
  join(main.run(), function(run) {
    console.log('layer business - run: ', run);
    return(run);
  }); // eof join main.run()
  
}) // eof then resourceForUuid
.catch(function(error) {
  console.log('layer business - error: ', error);
}) // eof catch
.finally(function() {
  console.log('layer business - finally');
}); // eof finally